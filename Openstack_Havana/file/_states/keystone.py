# -*- coding: utf-8 -*-
"""
Created on Mon Dec  9 10:54:06 2013
...
def endpoint_present(service=None,
                     publicurl=None,
                     internalurl=None,
                     adminurl=None,
                     profile=None):
    '''
    Ensure endpoint present for service
    ...
    ret = {'name': service,
           'changes': {},
           'result': True,
           'comment': 'endpoint for service "{0}" already exists'.format(service)}

    # Check if service is already present
    endpoint = __salt__['keystone.endpoint_get'](service)

    if endpoint:
        return ret
    else:
        # Create service
        __salt__['keystone.endpoint_create'](service,
                                             publicurl=publicurl,
                                             internalurl=internalurl,
                                             adminurl=adminurl,
                                             profile=profile)
        ret['comment'] = 'endpoint for service "{0}" has been added'.format(service)
        ret['changes']['endpoint'] = 'Created'
    return ret


def endpoint_absent(service):
    '''
    Ensure that the service doesn't exist in Keystone catalog
    ...
    ret = {'name': service,
           'changes': {},
           'result': True,
           'comment': 'endpoint for service "{0}" is already absent'.format(service)}
    ...
    if not endpoint:
        return ret
        # Delete service
    __salt__['keystone.endpoint_delete'](service_name=service)
    ret['comment'] = 'endpoint for service "{0}" has been deleted'.format(service)

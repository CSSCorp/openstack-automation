...
def __grant_generate(grant,
    ...
    identity = ''
    if password:
        identity = " identified by '{0}'".format(password)
    qry = 'GRANT {0} ON {1}.{2} TO {3!r}@{4!r}{5}'.format(
        grant, dbc, table, user, host, identity
    )
    if salt.utils.is_true(grant_option):
        pass#qry += ' WITH GRANT OPTION'
...
def grant_add(grant,
    ...
    qry = __grant_generate(grant, database, user, host, password, grant_option, escape)
    log.debug('Query: {0}'.format(qry))
    print qry
    try:
        cur.execute(qry)
